...
async def show_admin_tickets(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    header_text = (
        texts.t('ADMIN_TICKETS_TITLE_OPEN', '🎫未解决的支持工单：')
        if scope == 'open'
        else texts.t('ADMIN_TICKETS_TITLE_CLOSED', '🎫已关闭的支持工单：')
    )
...
async def view_admin_ticket(
    ...
    try:
        if ticket.user:
            admin_profile_btn = types.InlineKeyboardButton(
                text='👤致用户', callback_data=f'admin_user_manage_{ticket.user.id}_from_ticket_{ticket.id}'
            )
    ...
    try:
        if ticket.user and ticket.user.telegram_id and ticket.user.username:
            ...
            buttons_row.append(types.InlineKeyboardButton(text='✉ 下午', url=pm_url))
            profile_url = f'tg://user?id={ticket.user.telegram_id}'
            buttons_row.append(types.InlineKeyboardButton(text='👤 简介', url=profile_url))
            if buttons_row:
...
async def reply_to_admin_ticket(callback: types.CallbackQuery, state: FSMContext, db_user: User):
    ...
    await callback.message.edit_text(
        texts.t('ADMIN_TICKET_REPLY_INPUT', '请输入支持团队的回复：'),
    ...
    try:
        ...
        if not data.get('reply_mode'):
            try:
                ...
                await message.answer('❌ 输入整数分钟')
                return
            until = datetime.now(UTC) + timedelta(minutes=minutes)
            ok = await TicketCRUD.set_user_reply_block(db, ticket_id, permanent=False, until=until)
            if ok:
                await message.answer(f'✅ 用户被屏蔽 {minutes} 分钟')
            else:
                await message.answer('❌ 锁定错误')
            await state.clear()
            return
        ...
        await message.answer(
            texts.t('ADMIN_TICKET_REPLY_SENT', '✅回复已发送！'),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.t('VIEW_TICKET', '👁️查看工单'),
                            callback_data=f'admin_view_ticket_{ticket_id}',
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
                            text=texts.t('BACK_TO_TICKETS', '⬅️返回工单'), callback_data='admin_tickets'
                        )
                    ],
                ]
            ),
...
async def mark_ticket_as_answered(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    try:
        ...
        if success:
            texts = get_texts(db_user.language)
            await callback.answer(
                texts.t('TICKET_MARKED_ANSWERED', '✅工单已标记为已回复。'), show_alert=True
            )
            ...
            texts = get_texts(db_user.language)
            await callback.answer(texts.t('TICKET_UPDATE_ERROR', '❌更新工单时出错。'), show_alert=True)
...
async def close_all_open_admin_tickets(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if closed_count == 0:
        await callback.answer(
            texts.t('ADMIN_CLOSE_ALL_OPEN_TICKETS_EMPTY', 'ℹ️没有可关闭的未解决工单。'), show_alert=True
        )
        return
    ...
    success_text = texts.t('ADMIN_CLOSE_ALL_OPEN_TICKETS_SUCCESS', '✅已关闭未解决工单：{count}').format(
        count=closed_count
    )

    notification_keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[[types.InlineKeyboardButton(text='🗑 删除', callback_data='admin_support_delete_msg')]]
    )
        ...
        if success:
            ...
            try:
                await callback.message.answer(
                    ...
                    reply_markup=types.InlineKeyboardMarkup(
                        inline_keyboard=[
                            [types.InlineKeyboardButton(text='🗑 删除', callback_data='admin_support_delete_msg')]
                        ]
                    ),
                )
            except Exception:
...
async def block_user_in_ticket(callback: types.CallbackQuery, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        pass
    await callback.message.edit_text(
        texts.t('ENTER_BLOCK_MINUTES', '请输入封锁用户的分钟数（例如15）：'),
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
        ...
        try:
            ...
            try:
                if updated.user and updated.user.telegram_id and updated.user.username:
                    ...
                    buttons_row = []
                    pm_url = f'tg://resolve?domain={safe_username}'
                    buttons_row.append(types.InlineKeyboardButton(text='✉ 写信给PM', url=pm_url))
        ...
        if last_group_message:
            try:
                ...
                await callback.message.bot.send_message(
                    ...
                    text=texts.t('ATTACHMENTS_SENT', '✅附件已发送。'),
                    reply_markup=kb,
                )
            except Exception:
