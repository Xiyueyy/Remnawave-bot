...
async def _build_overview(
    ...
    if pages:
        ...
        for index, page in enumerate(pages, start=1):
            ...
            if not title:
                title = texts.t('FAQ_PAGE_UNTITLED', '无标题')
            if len(title) > 60:
                title = f'{title[:57]}...'
    ...
    header = texts.t('ADMIN_FAQ_HEADER', '❓<b>常见问题</b>')
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_HTML_HELP', 'ℹ️标记帮助'),
...
async def process_new_faq_content(
    ...
    title = data.get('faq_title') or texts.t('FAQ_PAGE_UNTITLED', '无标题')
    ...
    logger.info('管理员创建常见问题解答页面（字符）', telegram_id=db_user.telegram_id, content_count=len(content))
...
async def show_faq_page_details(
    ...
    header = texts.t('ADMIN_FAQ_PAGE_HEADER', '📄<b>常见问题页面</b>')
    title = (page.title or '').strip() or texts.t('FAQ_PAGE_UNTITLED', '无标题')
    status_label = texts.t(
        'ADMIN_FAQ_PAGE_STATUS_ACTIVE',
        '✅活跃',
    )
    ...
    if preview:
        ...
        if len(preview) > 400:
            preview_trimmed += '...'
        preview_text = texts.t('ADMIN_FAQ_PAGE_PREVIEW', '<b>预览：</b>\n{content}').format(
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_EDIT_TITLE_BUTTON', '✏️编辑标题'),
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_EDIT_CONTENT_BUTTON', '📝编辑文本'),
                callback_data=f'admin_faq_edit_content:{page.id}',
            )
        ]
    )

    toggle_text = texts.t('ADMIN_FAQ_PAGE_ENABLE_BUTTON', '✅启用页面')
    if page.is_active:
        toggle_text = texts.t(
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_PAGE_MOVE_UP', '⬆️上移'),
                callback_data=f'admin_faq_move:{page.id}:up',
            ),
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_PAGE_MOVE_DOWN', '⬇️下移'),
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_PAGE_DELETE_BUTTON', '🗑️删除'),
    ...
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_FAQ_BACK_TO_LIST', '⬅️返回常见问题设置'),
                callback_data='admin_faq',
            )
        ]
    )
...
async def process_edit_faq_title(
    ...
    if not page_id:
        await state.clear()
        await message.answer(texts.t('ADMIN_FAQ_UNEXPECTED_STATE', '⚠️状态已重置。'))
        return
    ...
    if not page:
        await message.answer(
            texts.t('ADMIN_FAQ_PAGE_NOT_FOUND', '⚠️未找到页面。'),
        )
        await state.clear()
        return
    ...
    await message.answer(
        texts.t('ADMIN_FAQ_TITLE_UPDATED', '✅标题已更新。'),
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
...
async def process_edit_faq_content(
    ...
    await message.answer(
        texts.t('ADMIN_FAQ_CONTENT_UPDATED', '✅页面文本已更新。'),
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
...
async def delete_faq_page(
    ...
    await callback.answer(
        texts.t('ADMIN_FAQ_PAGE_DELETED', '🗑️页面已删除。'),
        show_alert=True,
    )
...
async def move_faq_page(
    ...
    await callback.answer(
        texts.t('ADMIN_FAQ_PAGE_REORDERED', '✅顺序已更新。'),
        show_alert=True,
    )
    await show_faq_page_details(callback, db_user, db)
