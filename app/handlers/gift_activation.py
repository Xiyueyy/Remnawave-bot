...
async def handle_gift_activate(callback: types.CallbackQuery) -> None:
    ...
    if isinstance(callback.message, InaccessibleMessage):
        await callback.answer('该消息已过时。尝试/start。', show_alert=True)
        return
    ...
    await callback.message.edit_text('⏳ 我们激活礼物...', parse_mode=None)
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            if exc.status_code >= 500:
                await callback.message.edit_text('激活期间发生错误。请稍后重试。', parse_mode=None)
            else:
                await callback.message.edit_text(
                    f'激活礼物失败：{html_mod.escape(exc.message)}',
                    parse_mode=None,
                )
            return
        except Exception:
